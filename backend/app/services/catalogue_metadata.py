...
CATALOGUE_SCHEMA_VERSION = "2026-09-16.task-metadata.v1"
    ...
    payload.update(
        {
            ...
            "requires_approval": _bool_or_default(
                ...
                else merged.get("requires_approval"),
                default=risk in {"medium", "high"},
            ),
            "when_to_use": _clean_text(
                payload.get("when_to_use")
                or merged.get("when_to_use")
                or defaults.get("when_to_use")
            ),
            "metadata_source": "tasks/task_metadata.json",
            "metadata_schema_version": str(
                catalogue.get("schema_version") or CATALOGUE_SCHEMA_VERSION
            ),
        }
    )
    return payload
...
def _generated_task_description(item: dict[str, Any], merged: dict[str, object]) -> str:
    title = _clean_text(
        item.get("label") or item.get("title") or item.get("name") or item.get("id") or "This task"
    )
