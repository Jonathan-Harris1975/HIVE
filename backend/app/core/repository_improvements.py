...
async def _run_job(
    ...
    try:
        ...
        # The target-miss path above proves these values are present before
        # accepting best validated progress.  Bind non-optional views here so
        # the runtime invariant is also explicit to static type checking.
        validated_result = cast(dict[str, Any], selected_validation)
        improvement_payload = cast(dict[str, Any], selected_payload)
        qa_after = cast(dict[str, Any], validated_result["qa"])
        security_validation = cast(
            dict[str, Any], validated_result["security_validation"]
        )
        summary = str(
            improvement_payload.get("summary")
            or f"Automated improvements for {repository_id}"
        )
        raw_risks = improvement_payload.get("remaining_risks")
        remaining_risks = (
        ...
        raw_remaining_findings = improvement_payload.get("remaining_findings")
        remaining_findings = (
