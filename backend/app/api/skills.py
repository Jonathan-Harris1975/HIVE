"""Authenticated API for HIVE's repository-local skill catalogue."""
...
class SkillIndexReloadRequest(BaseModel):
    dry_run: bool = True
...
def skills_status(settings: Settings = Depends(get_settings)) -> dict[str, object]:
    """Return repository-local skill catalogue status."""
...
def rebuild_index(
    payload: SkillIndexReloadRequest,
    settings: Settings = Depends(get_settings),
) -> dict[str, object]:
    """Reload and validate the bundled catalogue without external writes."""

    return rebuild_skills_index(
        settings=settings,
